...
void inputDef(FILE* dest, char* Line, Fifo *head){
	...
	for(int n=0; n<strlen(Line); n++){
		if(Line[n] != ' ') // bosluga kadar kismi yazma
			fputc(Line[n],dest);
		else if (Line[n] == ' '){ // bsoluk ve registerlardan gelenleri yazma				//mehmetkose-toprakalp-gtu
			fputc(' ',dest);
			int q=0;
			while (iter->next != NULL ){
				fputs(iter->outputReg,dest);
...
void outputDef(FILE* dest, char* Line, Fifo *head){
	...
	for(int n=0; n<strlen(Line); n++){
		if(Line[n] != ' ')
			...
			while (iter->next != NULL ){
				fputs(iter->inputReg,dest);
				fputc(',',dest);	q += 2+ strlen(iter->inputReg) + strlen(iter->inputReg);
					iter = iter->next;
					if ( q >80){
						 fputc('\n',dest);
						 q =0;
...
int	Sperate( char *Line){
	char *temp = (char*)malloc(sizeof(char)*20);
	int i = 0, run = 0, write = 0; //write permssion to writeLine()  //mehmetkose-toprakalp-gtu
	...
	for(int n=0; n<strlen(Line); n++){
		if (  Line[n] != ' ' && Line[n] != '(' && Line[n] != ')'&& Line[n] != ','
			...
			if (run == 1 )
				write = 1; //dff bulundu
			for ( int q =0; q<strlen(temp); q++) //for ( k =0;k<sizeof(temp); k++)	temp[k] = '\0';
				temp[q] = ' ';
			i=0;
		}
	}
...
int seekAndWrite(char* str, Fifo* head){
	...
	while(iter->next != NULL){
		int ar;
	if( strcmp(str, "g18 ") == 0) { // printf("str=<%s>  input=<%s>   output=<%s>\n",str,iter->inputReg,iter->outputReg ); scanf("%d",&ar);// 
		}
